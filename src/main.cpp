...
#if defined(NDEBUG)
# error "Bahiti cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x5ab2d4a5e4a5cb14a17960269875f56fd950b36b4d420fe4d8441d89d154d115");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Bahiti: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Bahiti Signed Message:\n";
...
{
    // Bahiti: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 300000); // Bahiti: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 8 * 24 * 60 * 60; // Bahiti: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Bahiti: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Bahiti: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Bahiti: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xa1938b2b714d2debf34de2b98bf5ddda4f8f570ddb0fe6a69f94f933e44a03f3");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is bahiti since 18/10/2018";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0491387a1eb4e22ec76c69484437eb2cb296c5a322df87d2dfb1663c5eefcaa0bcd676a56e4eecba279daa5bacc25994725d257c811f7f63c24e72f1b7b6c4795e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1539884176;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 478100;

        if (fTestNet)
        {
            block.nTime    = 1539884167;
            block.nNonce   = 570963;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xd8860800ceeac5c85424653a89755f25c440d5207ed5f5413e336b041b94401e"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf9, 0xcb, 0xb8, 0xda }; // Bahiti: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BahitiMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BahitiMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BahitiMiner(CWallet *pwallet)
{
    printf("BahitiMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("bahiti-miner");
    ...
    try { loop {
        ...
        printf("Running BahitiMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BahitiMiner, pwallet));
}
